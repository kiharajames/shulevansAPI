...
$target_dir = SCHOOL_PATH."/img/profile_images/".$adm_no."/";
$newname = md5(rand(1,1000));
$target_file = $target_dir . $newname;
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($_FILES["profileImage"]["name"],PATHINFO_EXTENSION));
$target_file = $target_dir . $newname.".".$imageFileType;
$file_to_upload = $adm_no."/".$newname.".".$imageFileType;
...
if ($_FILES["profileImage"]["size"] > 5000000) {
    ...
    && $imageFileType != "gif" ) {
        ...
        if ((move_uploaded_file($_FILES["profileImage"]["tmp_name"], $target_file)!== false)) {
            $sql = "UPDATE users SET avatar=:target_file WHERE adm_no=:adm_no";
